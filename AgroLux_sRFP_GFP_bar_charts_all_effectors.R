### Horizontal bar chart of AgroLux, sRFP and GFP values for all RXLR 
### effectors (Figure S2)
### By Freddie King
### 2021/12/09
...
microplate_dat <- read.csv(file.choose())
...
# Dunnett's test on blocked two-way ANOVAs
...
# Adding p-value asterisks to microplate dataframe
rfp_pvalues <- rbind(c(" "), rfp_pvalues)
rfp_pvalues$?..treatment <- sort(summ_dat$?..treatment)
...
p_lux <- ggplot() +
  ...
  geom_text(data=summ_dat, aes(label=Ev.y, x=reorder(?..treatment, -lux_mean),
            y = lux_mean+(lux_mean*1.85)), vjust = 0.75,
            position = position_dodge(width = 0.5), size=6) +
...
p_gfp <- ggplot() +
  ...
  geom_bar(data=subset(summ_dat, ?..treatment == "Ev"), aes(y=gfp_mean, x=?..treatment),
           fill="grey70", stat="identity",
           width=0.5, alpha=1, show.legend = FALSE) +
